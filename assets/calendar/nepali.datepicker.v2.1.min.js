...
function showNdpCalendarBox(a)
{
    ...
    else{ndpData[a]&&(ndpAttr=ndpData[a]);
        var b=jQuery("#"+a).val();
        jQuery("#ndp-target-id").html(a);
        var c=jQuery("#"+a).offset();
        jQuery("#ndp-nepali-box").css("top",c.top+jQuery("#"+a).outerHeight()),jQuery("#ndp-nepali-box").css("left",c.left),showCalendar(b,a),npdCalendarVisible=!0}
}
    function setSelectedDay(a,my_sel_id)
    {
        ...
        var b=jQuery("#ndp-target-id").html();jQuery("#"+b).val(a),ndpAttr.ndpEnglishInput&&jQuery("#"+ndpAttr.ndpEnglishInput).val(BS2AD(a)),hideCalendarBox()
    }
    function showCalendar(a,my_sel_id)
    {
        window.my_sel_id = my_sel_id;
        jQuery("#ndp-nepali-box table").find("tr:gt(0)").remove(),""===a?jQuery("#ndp-nepali-box table").append(getDateTable("")):jQuery("#ndp-nepali-box table").append(getDateTable(a)),"block"==jQuery("#ndp-nepali-box").css("display")&&jQuery("#ndp-nepali-box").hide(),jQuery("#ndp-nepali-box").fadeIn(100)
    }
    function getDateTable(a)
    {
        ...
        if(""===a)
        {
            var d=getNepaliDate();return b=getMonthParameters(d),c=getDateRows(b[0],b[1],b[2],b[3],b[4])}return b=getMonthParameters(a),c=getDateRows(b[0],b[1],b[2],b[3],b[4])
        }
        function performSelectMonth(a)
        {
            showCalendar((ndpAttr.npdYear?jQuery("#npd-year-select").val():a)+"-"+jQuery("#npd-month-select").val()+"-01")}function performSelectYear(a){showCalendar(jQuery("#npd-year-select").val()+"-"+(ndpAttr.npdMonth?jQuery("#npd-month-select").val():a)+"-01")}function getMonthSelect(a,b){var c=getNepaliMonthsInNepali(),d='<select id="npd-month-select" onchange="performSelectMonth('+b+')">';return c.forEach(function(b,c){d+='<option value="'+(c+1)+'"'+(c+1==a?" selected":"")+">"+b+"<li>"}),d+="<select>"}function getYearSelect(a,b,c){console.log(c);var d='<select id="npd-year-select" onchange="performSelectYear('+a+')">';for(i=Math.round(c/2)>0&&parseInt(b)-Math.round(c/2)>=2e3?parseInt(b)-Math.round(c/2):2e3;i<=(Math.round(c/2)>0&&parseInt(b)+Math.round(c/2)<=2090?parseInt(b)+Math.round(c/2):2090);i++)d+='<option value="'+i+'"'+(b==i?" selected":"")+">"+englishNo2Nep(i)+"<li>";return d+="<select>"
        }
        function getMonthParameters(a)
        {
            var b=a.split("-"),c=b[0],d=b[1],e=ndpAttr.npdYearCount||0;ndpAttr.npdMonth&&ndpAttr.npdYear?jQuery("#currentMonth").html(getMonthSelect(d,c)+getYearSelect(d,c,e)):ndpAttr.npdMonth?jQuery("#currentMonth").html(getMonthSelect(d,c)+" "+englishNo2Nep(c)):ndpAttr.npdYear?jQuery("#currentMonth").html(getNepaliMonth(d-1)+" "+getYearSelect(d,c,e)):jQuery("#currentMonth").html(getNepaliMonth(d-1)+"&nbsp;"+englishNo2Nep(c)),nYear=pYear=c,nMonth=parseInt(d,10)+1,nMonth>12&&(nYear++,nMonth="01"),pMonth=parseInt(d,10)-1,pMonth<1&&(pYear--,pMonth="12"),jQuery("#prev").attr("onclick","showCalendar('"+pYear+"-"+pMonth+"-01')"),jQuery("#next").attr("onclick","showCalendar('"+nYear+"-"+nMonth+"-01')");var f=b[2],g=numberOfBsDays(c,d-1),h=new NepaliDateConverter,i=d+"/1/"+c,j=h.bs2ad(i),k=j.split("-"),l=k[0],m=k[1],n=k[2],o=new Date(l,m-1,n),p=o.getDay();return new Array(p,g,c,d,f)
        }
        function getDateRows(a,b,c,d,e)
        {
            for(var f=getNepaliDate(),g=f.split("-"),h=g[0],i=get2DigitNo(g[1]),j=get2DigitNo(g[2]),k="",l=0;a+b>l;l++){l%7===0&&(k+="<tr>");var m=l-a+1,n=c.toString()+"-"+get2DigitNo(d)+"-"+get2DigitNo(m),o="";o=c==h&&get2DigitNo(d)==i&&j==get2DigitNo(m)?"ndp-selected":m==e?"ndp-current":"ndp-date",a>l?k+="<td></td>\n":(k+="<td class='"+o+"'>",k+="<a href='javascript:void(0)' onclick=\"setSelectedDay('"+n+","+my_sel_id+"')\">"+englishNo2Nep(m)+"</a>",k+="</td>\n"),l%7==6&&(k+="</tr>\n")}return k}function hideCalendarBox(a){a="undefined"!=typeof a?a:!0,jQuery("#ndp-nepali-box").fadeOut(100),npdCalendarVisible=!1,a&&ndpAttr.onChange&&ndpAttr.onChange()
        }
